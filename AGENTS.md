...
- Auth (Story 1.5): **Better Auth 1.7.x**, Google provider only (`emailAndPassword` off). `src/auth/auth.ts` (server instance) · `src/lib/auth-client.ts` (`createAuthClient`, browser) · `src/app/api/auth/[...all]/route.ts` (`toNextJsHandler`). Tables `user`/`session`/`account`/`verification` (`@@map` lowercase; models stay `PascalCase`). `User` table renamed from `User` → `user`; `googleSub` dropped (OAuth identity now lives in `account`). `isAdmin` is a Better Auth `additionalFields` entry with `input: false` — on `session.user`, set only by the Story 1.7 action. Env: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `BETTER_AUTH_SECRET` (≥32 chars), `BETTER_AUTH_URL`. Google Cloud redirect URI: `<BETTER_AUTH_URL>/api/auth/callback/google`.
