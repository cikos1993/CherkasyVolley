...
- Epic 2 schema (Story 2.1, migration `20260903174727_tournament_schema`): `tournament` / `team` / `tournament_entry` / `player` + enums `Discipline` / `TournamentType` / `TournamentState` / `ScoringPreset`. `Team.name` is `@unique`; `TournamentEntry` is `@@unique([tournamentId, teamId])`; `Player` needs only `fullName`, the rest are optional `String?`. Deletes: tournament→entries→players cascade; a `team` with entries is `onDelete: Restrict`. `Group` / `GroupSlot` / `Match` / `SetScore` come in Epic 3.
