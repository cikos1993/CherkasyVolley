...
Implementation review 2026-09-04 (`bmad-code-review`, 4 layers: Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor) over `git diff f202fae..HEAD` (after the `/code-review` fix pass, commit `a2ddc7e`). Acceptance Auditor: all 4 ACs met, "substantially compliant". 0 decision-needed, 5 patch, 3 defer, 12 dismissed (one confirmed false positive).

#### Patch

- [x] **[Review][Patch] `AGENTS.md`'s own Story 2.4 line still describes the abandoned `defaultValue`/`state.values` mechanism** [AGENTS.md] — the `/code-review` pass fixed this exact stale claim in `src/actions/README.md` and `src/components/README.md` but missed the identical sentence in `AGENTS.md`'s Stack-status bullet. Confirmed by two independent layers (Blind Hunter, Acceptance Auditor). Update to describe the controlled-state form.
- [x] **[Review][Patch] `AGENTS.md`'s Story 2.1 line ("`Group` … come in Epic 3") now contradicts its own Story 2.4 line three lines below** [AGENTS.md] — `Group` shipped in Story 2.4 (Epic 2), not Epic 3. Reword the older line to drop `Group` (keep `GroupSlot`/`Match`/`SetScore` as Epic 3).
- [x] **[Review][Patch] `isUniqueViolation` treats any `P2002` as "duplicate name", without checking which constraint fired** [src/data/tournaments.ts] — a `P2002` from `group_tournamentId_key` (or any future unique constraint) would surface the same misleading "Турнір з такою назвою вже існує" message. Confirmed by two independent layers (Blind Hunter, Edge Case Hunter). **Discovery while fixing:** the classic Prisma `error.meta.target` (array/string of column names) does not appear at all under the `@prisma/adapter-pg` driver adapter (Prisma 7) — the actual shape is `error.meta.driverAdapterError.cause.constraint.index` (the Postgres index **name**, verified live against Neon: `"tournament_discipline_type_year_name_key"`). `isUniqueViolation(error, indexName?)` now checks both shapes; the index name is a named export (`TOURNAMENT_NATURAL_KEY_INDEX`) rather than a string literal at the call site. Re-verified live (not just typechecked) via the extended `verify-tournament-create.mts`.
- [x] **[Review][Patch] `src/data/README.md` misstates `getTournamentForAdmin`'s signature** [src/data/README.md] — says it "takes the `NewTournamentInput` type from `src/domain`"; only `createTournamentRecord` does — `getTournamentForAdmin(id: string)` takes just an id. Cosmetic wording fix.
- [x] **[Review][Patch] `verify-tournament-create.mts` only asserted `state`/`scoringPreset`, never read back `name`/`year`/`type`/`teamCount`/`rounds`** [scripts/verify-tournament-create.mts] — a field-transposition regression in `createTournamentRecord`'s `data:` object (e.g. `teamCount`/`rounds` swapped) would have passed every existing check. Extended to assert the full round-trip — 13/13 checks pass live.

#### Defer

- [x] **[Review][Defer] No admin listing page for tournaments** — after one redirect, an admin has no in-app way to find a previously-created tournament except the URL. Expected: a list belongs to whichever story adds tournament management surfaces (Story 2.5 "edit/delete" is the natural owner, needs a list to pick a tournament to edit).
- [x] **[Review][Defer] `createTournament` does not `revalidatePath`, unlike its sibling `transitionTournament`** — no cached page depends on it yet (no admin list exists). Revisit once one does.
- [x] **[Review][Defer] `TournamentForm`'s field-error rendering has no component test** (no test exercises that a given `fieldErrors` key renders under the right field with correct `aria-describedby`) — same class as the already-tracked "no component-test toolchain" gap (jsdom/testing-library not installed); fold in when that toolchain lands.

#### Dismissed as noise / already tracked / by design (12)

`validateNewTournament`'s `discipline`/BEACH branches unreachable via the real form (intentional defensive completeness, AD-9) · `Tournament.name` not normalized against the unique constraint (already tracked in `deferred-work.md`) · migration adds the unique index with no pre-check for existing collisions (table is empty — verified via `db-check`) · verify script doesn't directly exercise `Group`'s own unique constraint (transitively covered by every `createTournamentRecord` call) · `Field`'s `onChange` prop typed structurally rather than as `React.ChangeEventHandler` (deliberate — unifies `<input>`/`<select>` binding) · `initialValues()` leaves `teamCount` empty while other fields default (no sensible universal team-count default exists) · no compile-time link between the domain unions and the Prisma enums (matches the `tournamentState.ts` precedent already accepted) · `createTournament` has no automated test (already tracked in `deferred-work.md`) · TOCTOU between `requireAdmin()` and the write if role is revoked mid-request (same risk tolerance as the rest of the codebase) · a forged POST could submit a File blob as `name`, coercing to `"[object File]"` (not exploitable, no crash, requires bypassing the real form) · the story's own "Files this story touches" table / AC-interpretation notes still describe the pre-pivot `defaultValue` plan (a planning artifact — the Change Log already documents the pivot; rewriting the historical AC notes would erase the decision record) · **`bind()` not returning a `name` prop, allegedly leaving every field unnamed (Edge Case Hunter) — confirmed false positive**: every control sets `name="..."` directly in JSX (verified via `grep 'name=' src/components/tournament-form.tsx`); the reviewing subagent worked from an abbreviated version of the file that omitted this.
...
| 2026-09-04 | `/code-review` (built-in skill, 8 finder angles). 10 findings; fixed 6: `[id]/page.tsx` `generateMetadata` read admin-only data ahead of the layout's auth gate (removed, static title); `tournament-form.tsx` `maxLength` now imports `NAME_MAX`; the `formError` toast effect keyed on `state` (was `formError` alone — two identical error strings in a row would not re-fire it); `INITIAL` year moved from module scope into a per-mount initializer; `verify-tournament-create.mts` now cleans up the duplicate-check row if the unique constraint ever regresses; `src/actions/README.md` + `src/components/README.md` corrected (`values`/`defaultValue` → the actual controlled-state mechanism). 1 tracked in `deferred-work.md` (`Tournament.name` not normalized against the exact-string unique). 3 dismissed as by-design (`Group?` nullable relation, `tournament-labels.ts` location, `selectClassName` duplication). Gate re-run clean: `test` 39/39, `typecheck`, `lint`, `build`, verify script 8/8. |
| 2026-09-04 | `bmad-code-review` (4 layers) over the post-`/code-review` diff. 5 patch, 3 defer, 12 dismissed (1 confirmed false positive — Edge Case Hunter's "`bind()` has no `name`" claim; every control sets `name` directly in JSX). Fixed: `AGENTS.md`'s two stale lines (Story 2.4 still described the abandoned `defaultValue` mechanism; Story 2.1 said "`Group` … Epic 3" contradicting the Story 2.4 line below it); `src/data/README.md` corrected `getTournamentForAdmin`'s signature; `verify-tournament-create.mts` now round-trips every field (`name`/`year`/`type`/`teamCount`/`rounds`), not just `state`/`scoringPreset`. `isUniqueViolation` redesigned to narrow by constraint — **discovered live that `@prisma/adapter-pg` (Prisma 7) doesn't populate the classic `error.meta.target`**; the real shape is `error.meta.driverAdapterError.cause.constraint.index` (the Postgres index name). `isUniqueViolation(error, indexName?)` checks both shapes; `TOURNAMENT_NATURAL_KEY_INDEX` is a named export, not a literal at the call site. 3 items added to `deferred-work.md` (no admin tournament list; `createTournament` has no `revalidatePath`; no component test for field-error rendering). Gate re-run clean: `test` 39/39, `typecheck`, `lint`, `build`; verify script 13/13. |
