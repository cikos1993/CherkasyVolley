...
_Code review (`bmad-code-review`, 4 layers: Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor) over `git diff 327854a..c186327` (`src/` + `scripts/`). 0 decision-needed, 6 patch → all applied, 7 deferred, ~10 dismissed. Acceptance Auditor: no blocking AC violation, all 10 tasks present. Gate re-run clean: `build` / `typecheck` / `lint`, `pnpm test` 169/169, all 17 verify scripts green, `migrate diff` empty._

#### Patch

- [x] [Review][Patch] Archive detail section order contradicts the established order `[src/app/archive/[year]/[tournament]/page.tsx]` — the page renders Команди → Таблиця → Розклад → Плейоф, but DESIGN §176 / `tournament-tabs.tsx` (Story 3.8) fix the order as **Таблиця → Розклад → Команди → Плейоф**. Reorder the four `<section>`s to match. (blind-hunter)
- [x] [Review][Patch] `/archive` list uses `Promise.all` — one corrupt tournament 500s the whole index `[src/app/archive/page.tsx]` — `getPlayoffBracket` → `advanceBracket`'s `indexBySlot` throws on a duplicate `(tournamentId, slot)` row (an un-constrained, known-deferred case). `Promise.all` rejects wholesale and there is no `error.tsx`, so a single bad row takes down all of `/archive`. Switch to `Promise.allSettled` and skip a rejected tournament (leave its places line blank / omit it) — contains the blast radius to one row. (blind-hunter + edge-case-hunter)
- [x] [Review][Patch] `resolve()` accepts non-canonical `year` strings `[src/app/archive/[year]/[tournament]/page.tsx:20]` — `tournament.year !== Number(year)` passes for `"2026.0"`, `"02026"`, `"+2026"`, `" 2026 "` → several URLs render the same page with no canonical. Tighten to `if (!/^\d+$/.test(year) || tournament.year !== Number(year)) return null;`. (blind-hunter + edge-case-hunter + verification-gap)
- [x] [Review][Patch] No `/archive/[year]` route — `/archive/2025` is a guessable 404 `[src/app/archive/[year]/]` — the year segment is in every record URL and the `/archive` group headings are bare years. Add a one-line `src/app/archive/[year]/page.tsx` that `redirect("/archive")` (the story listed this as optional polish; two reviewers flagged the guessable 404). (blind-hunter)
- [x] [Review][Patch] The archive record header shows only the name — no type / year for a deep-link `[src/app/archive/[year]/[tournament]/page.tsx]` — a visitor landing directly on `/archive/2026/<id>` sees no year/type context (both live only on the list). Add a muted meta line `{TOURNAMENT_TYPE_LABELS[tournament.type]} · {tournament.year}` under the `<h1>` (`getArchivedTournament` already returns the full row, so `type`/`year` are available). (blind-hunter + acceptance-auditor)
- [x] [Review][Patch] `verify-archive.mts` coverage holes `[scripts/verify-archive.mts]` — (a) `publicScheduleRows` (the extracted schedule mapper, used by `/classic` schedule tab + the archive Розклад section) has **no test anywhere** — add a hand-built `fakeMatches` block (mirror the `fakeStandings` pattern) asserting the `homeEntry`/`awayEntry` → `homeTeam`/`awayTeam` order, the `"—"` fallback, the `scheduledAt ? formatted : null` branch, and `resultSummary` from `matchScoreLabel`; (b) the `standingsTableRows` block asserts only `position` + `qualifies` — extend it to assert `wins` / `losses` / `setsWon` / `setsLost` / `teamName` / `needsManualSeed` pass through (a transposition would ship green today); (c) add `getArchivedTournament(draft) === null`. (verification-gap + acceptance-auditor)

#### Defer

- [x] [Review][Defer] `advanceBracket`'s duplicate-slot throw on the anonymous `/archive` + `/archive/[year]/[tournament]` path `[src/app/archive/**]` — deferred, restated. `getPlayoffBracket` / `playoffPlacements` both run `indexBySlot`, which throws on a duplicate `(tournamentId, slot)` `Match` row; there is no `error.tsx` anywhere, so a corrupt row 500s the page. Unreachable via the sanctioned writers (`savePlayoffFormation` / `savePlayoffAdvancement` each create a slot once under a row lock); the real fix is the already-tracked `@@unique([tournamentId, slot])` partial-index schema follow-up (raw-SQL only, `migrate diff` drift risk). `Promise.allSettled` (patch above) contains the *list-page* blast radius; a route-segment `error.tsx` for `/archive` (and the app at large) is a broader hardening task. (blind-hunter + edge-case-hunter)
- [x] [Review][Defer] The reused `StandingsTable` / `PublicSchedule` / teams empty-states show draw-pending copy («Групу буде сформовано після жеребкування», `NO_TEAMS`) on the archive record `[src/app/archive/[year]/[tournament]/page.tsx]` — deferred, unreachable for a valid `COMPLETED` tournament (Story 4.5 gates `PLAYOFF → COMPLETED` on the full playoff, which requires the drawn+played group). Only a `setTournamentState`-forced `COMPLETED` (verify scripts) or a future bug hits it, and the same list-row «1. — · 2. — …» falls out. If a forced-`COMPLETED` path is ever exposed beyond tests, the archive pages need their own empty guards with archive-appropriate copy. (blind-hunter + edge-case-hunter + acceptance-auditor)
- [x] [Review][Defer] No `loading.tsx` for the two new routes `[src/app/archive/**]` — deferred, app-wide and pre-existing. No route in the app has a `loading.tsx`; the `Skeleton` / `TableSkeleton` primitives exist but a streaming-boundary pass is cross-cutting (2.9 / 3.8 reviews). Each archive page is a single Server Component render at v1 scale. (blind-hunter)
- [x] [Review][Defer] `resolve()` queries `getArchivedTournament` twice per record load (`generateMetadata` + body), no `React.cache` `[src/app/archive/[year]/[tournament]/page.tsx]` — deferred, matches the existing `/classic/[tournament]` pattern (`resolveTournament` called twice, 2.9 review). One extra indexed `findFirst`; a cross-cutting `cache()` pass covers both pages. (blind-hunter)
- [x] [Review][Defer] `getPublicTournament` still serves `COMPLETED` tournaments and the `/classic/[tournament]` copy has no `canonical`/`noindex` pointing at the archive `[src/data/tournaments.ts, src/app/classic/[tournament]/page.tsx]` — deferred, deliberate story decision (a shared `/classic/<completed>` link must keep resolving, with the Story 4.5 banner). The app has no SEO / robots / canonical infrastructure at all (2.9 review "no SEO/indexing discussion"); adding it is a cross-cutting task. A «← Архів» affordance on `/classic/[tournament]` when `COMPLETED` is a candidate follow-up. (blind-hunter)
- [x] [Review][Defer] `/archive` list runs one `getPlayoffBracket` per tournament (unbounded fan-out) `[src/app/archive/page.tsx]` — deferred, the story's own noted follow-up. A batched read (`match.findMany({ where: { tournamentId: { in: ids }, stage: { in: PLAYOFF_STAGES } } })` → group → `playoffPlacements` per group) replaces the loop when the archive holds dozens of tournaments. Note: `playoffPlacements` also runs `indexBySlot`, so a batched read is not a pure win over the throw either — `Promise.allSettled` is the pragmatic containment. Fine at v1 scale (a handful of tournaments per year, NFR-5). (blind-hunter)
- [x] [Review][Defer] No page/component test for the two new routes' markup, the `byYear` grouping, the `Promise.all` placements fetch, or the `resolve()` year-mismatch 404 `[src/app/archive/**]` — deferred, the standing "no component toolchain" gap. `verify-archive.mts` (with the patch above) covers every `src/data` read the pages depend on; the JSX is the documented manual pass. `resolve()` is a page-local function (not exported) so it is not verify-script-testable without restructuring. (verification-gap)
...
- `src/app/archive/[year]/page.tsx` (NEW — review patch: redirect to `/archive`)
...
| 2026-09-07 | Code review (`bmad-code-review`, 4 layers) — 0 decision-needed, 6 patch, 7 deferred, ~10 dismissed. All 6 patches applied: detail-page section order → Таблиця·Розклад·Команди·Плейоф (DESIGN §176); `/archive` list `getPlayoffBracket` per-tournament `.catch(() => null)` (a corrupt row no longer 500s the index; blank places line omitted); `resolve()` rejects non-canonical `year` (`/^\d+$/`); `src/app/archive/[year]/page.tsx` (NEW) → `redirect("/archive")`; «{type} · {year}» meta line under the record `<h1>`; `verify-archive.mts` now covers `publicScheduleRows`, all `standingsTableRows` fields, and `getArchivedTournament(draft)`. Gate re-run clean (`build`/`typecheck`/`lint`, `pnpm test` 169/169, all 17 verify scripts green, `migrate diff` empty). Status: done. |
