...
- [ ] **Task 2 — `src/domain/teamForm.ts` (NEW) + Vitest spec** (AC: 1, 2)
  ...
  - [ ] `teamNameKey(normalizedName: string): string` — case-fold (`.toLowerCase()`) the already-normalized name. This is the value stored in `nameKey`.
  - [ ] `NewTeamInput = { name: string }`; `TeamField = "name"`; `FieldErrors = Partial<Record<TeamField, string>>`.
  - [ ] `validateNewTeam(raw: { name: RawValue }): { ok: true; value: NewTeamInput } | { ok: false; fieldErrors: FieldErrors }` — normalizes via `normalizeTeamName`; empty → `"Вкажіть назву команди."`; over `TEAM_NAME_MAX` → a length message (mirror `tournamentForm.ts`'s wording style). Returns the **normalized** name in `value.name` (not the raw input) so the caller never has to re-normalize.
  - [ ] `src/domain/teamForm.test.ts` — empty/whitespace-only input; trims + collapses internal whitespace (`"  Спартак   Черкаси  "` → `"Спартак Черкаси"`); over-length rejected; `teamNameKey` case-folds (`"Спартак"` / `"спартак"` / `"СПАРТАК"` → identical key) and is stable under repeated whitespace collapse; message is a non-empty Ukrainian string.
  - [ ] `pnpm test` green (new file only — no changes to existing domain specs).
