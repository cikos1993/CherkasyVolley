...
- [ ] **Task 10 — Verification gate** (AC: all)
  ...
  - [ ] Import-boundary greps: no new Prisma import site outside `src/data/**`; confirm `src/components/status-badge.tsx`/`public-roster.tsx` don't import `@/auth` (only `src/app/**` pages do, per the sanctioned edge).
  - [ ] `scripts/verify-public-tournament.mts` (NEW, self-cleaning): create a `DRAFT` tournament + one `GROUP_STAGE` tournament (via `setTournamentState`, bypassing `transitionTournament`'s precondition the same way `verify-tournament-edit-delete.mts` already does for test setup) → assert `getPublicTournament`/`listPublicTournaments` see the `GROUP_STAGE` one and not the `DRAFT` one → assert `getEntryByTeam` finds a real entered team and returns `null` for a team never entered → assert `getEntryByTeam` scoped to a *different* tournament's id returns `null` even for a real `teamId` (the Story 2.7/2.8 cross-scoping lesson, applied here a third time) → full teardown.
  - [ ] Re-run all five prior verify scripts (`verify-tournament-create.mts`, `verify-tournament-edit-delete.mts`, `verify-team-create.mts`, `verify-team-enrollment.mts`, `verify-roster.mts`) — no regression.
  - [ ] **A real browser walkthrough — finally not blocked by the missing OAuth automation**, since every page this story adds requires no sign-in: `/classic` (empty and non-empty), `/classic/[tournament]` for a `GROUP_STAGE` tournament, its "Команди" tab, a team's `/classic/[tournament]/teams/[team]` roster, and a `DRAFT` tournament's direct link 404ing while signed out. The admin-preview fallback branch (signed in as admin, viewing a `DRAFT` tournament's public page) still needs a manual signed-in pass, same residual gap as every admin-touching flow since Story 2.4.
