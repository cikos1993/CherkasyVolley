...
- **`computeStandings` lives in `scoring.ts`, per `ARCHITECTURE-SPINE.md`'s own file map** ("scoring.ts — computeStandings, points-per-match за пресетом"); `tiebreak.ts` calls `scoring.ts`'s `computeStandings` for its own mini-table step (`tiebreak.ts → scoring.ts`, not the other way around — both directions are legal same-layer domain imports under AD-3, this just states which one is actually used).
...
**Code review — the real bug: `schedule.ts`'s circle-method "fixed" anchor entry was home in 100% of its matches, contradicting the story's own documented "arbitrary but stable" claim.** Found by Blind Hunter, confirmed by hand-tracing (whichever entry sits at `entryIds[0]` never rotates, so it's always `current[0]`, always paired first). Fixed by shuffling each individual pair's two elements too, reusing the same injectable `shuffle` already used to reorder pairs within a tour — a `reverseShuffle` regression test confirms home/away now genuinely tracks `shuffle`'s output instead of circle-method position.

**Code review — a second finding converged on independently by three layers (Edge Case Hunter, Verification Gap Reviewer, Blind Hunter): `validateMatchScore` checked decisiveness by array index but looked up the target score by `set.setNo`, with nothing verifying the two agree.** Fixed by rejecting any `sets` array whose `setNo` values aren't ascending, contiguous, and 1-indexed, before any other check runs.

**Code review — also fixed:** the duplicate "who won this set" comparison (`scoring.ts`'s private tally vs. `validation.ts`'s inline reimplementation) consolidated into one exported `homeWonSet` helper; `SetValidation`/`MatchValidation` (structurally identical) merged into one `Validation` type; a missing `validateMatchScore`-level test for an illegal-margin `CUSTOM` match added (the story's own headline "win-by-2 applies to both presets" decision was previously only tested at the lower `validateSetScore` level); two doc-comment gaps closed (`scoring.ts`'s trust-boundary assumptions, `tiebreak.ts`'s name-fallback sort direction); two inaccurate self-reported claims in this story file corrected (the backwards `scoring.ts`/`tiebreak.ts` call direction, and `orderStandings` mischaracterized as "recursive").
...
- **Task 2:** `src/domain/tiebreak.ts` (NEW) — `orderStandings`, the FR-17 chain via a fixed three-stage linear pipeline (`resolveTiedGroup` → `resolveBySetsWon` → `resolveByNameFallback`; points → mini-table → sets won → name). 4 Vitest cases including a genuine 3-way stats cycle.
...
- `src/domain/schedule.ts` — each pair's home/away is now also passed through `shuffle`, closing the "fixed anchor entry always home" bug
- `src/domain/schedule.test.ts` — new regression test proving home/away depends on `shuffle`, not circle-method position
- `src/domain/scoring.ts` — new exported `homeWonSet` helper (single source for "who won this set"); doc comments on `matchPoints`/`computeStandings` stating the preconditions they trust
- `src/domain/validation.ts` — `validateMatchScore` now rejects out-of-order/gapped `setNo`; reuses `homeWonSet` instead of reimplementing the comparison; `SetValidation`/`MatchValidation` merged into one `Validation` type
- `src/domain/validation.test.ts` — new tests: CUSTOM margin violation at the `validateMatchScore` level, out-of-order `setNo`, gapped `setNo`
- `src/domain/tiebreak.ts` — doc comment on the name-fallback's sort direction being an arbitrary-but-deterministic choice
- `_bmad-output/implementation-artifacts/3-1-domain-engine-scoring-tiebreak-schedule-validation.md` — corrected the backwards `scoring.ts`/`tiebreak.ts` call-direction claim and the "recursive" mischaracterization of `orderStandings`
