...
- [ ] **Task 3 — `src/actions/entries.ts` (NEW): `enrollTeam` + `removeTeamEntry`** (AC: 1, 2, 3)
  ...
  - [ ] Imports: `requireAdmin` (`@/auth/requireAdmin`), `getTournamentForAdmin` (`@/data/tournaments`), `countTournamentEntries`/`createEntry`/`deleteEntry`/`TOURNAMENT_ENTRY_NATURAL_KEY_INDEX` (`@/data/entries`), `isUniqueViolation`/`isRecordNotFound` (`@/data/errors`), `checkCanEnroll`/`checkCanRemoveEntry` (`@/domain/teamEnrollment`), `toActionError`/`ActionResult` (`@/actions/result`), `revalidatePath` (`next/cache`). No new `ActionErrorCode` — `PRECONDITION_FAILED` and `NOT_FOUND` already exist (Story 2.3).
- [ ] **Task 4 — `src/components/team-enrollment.tsx` (NEW, Client Component)** (AC: 1, 2, 3)
  - [ ] `"use client"`. Props: `{ tournamentId: string; state: TournamentState; teamCount: number; entries: { id: string; teamId: string; team: { id: string; name: string } }[]; availableTeams: { id: string; name: string }[] }` (the page computes `availableTeams` = `listTeams()` minus already-enrolled `teamId`s, and passes `entries.length` implicitly via `entries`).
  - [ ] **Enroll control** — a native `<select>` of `availableTeams` + a submit `Button` ("Заявити"), `useTransition` + direct call to `enrollTeam.bind(null, tournamentId)` (the `GrantAdminButton` pattern: `startTransition(async () => { const res = await enrollTeam(tournamentId, teamId); if (res.ok) { notify.success("Команду заявлено"); router.refresh(); } else notify.error(res.message); })`). Disabled (with a caption reusing the domain's own message — call `checkCanEnroll` client-side too, purely for the UI hint, not as an authorization boundary) when `state !== "DRAFT"` or `entries.length >= teamCount` or `availableTeams.length === 0`.
  - [ ] **Empty-directory case** — if `availableTeams.length === 0` **and** `entries.length === 0` (nothing enrolled yet and the directory itself is empty), replace the `<select>` with a line: `Немає жодної команди в довіднику.` + a link to `/admin/teams` ("Додати команду"). If `availableTeams.length === 0` but some teams **are** already enrolled (every directory team is already in this tournament), disable the control with a caption instead — the directory isn't empty, it's just exhausted.
  - [ ] **Entries list** — `<ul>` of `entries`, each row: team name + (when `state === "DRAFT"`) a `ConfirmDialog`-gated "Зняти" button (title "Зняти заявку команди?", description naming the team, `confirmLabel="Зняти"`, `destructive`) calling `removeTeamEntry(tournamentId, entry.id)`, same shape as `DeleteTournamentButton`. Outside `DRAFT`, render the row as plain text (no button) — matches the AC-interpretation decision that removal is `DRAFT`-only, and there is nothing useful a disabled button would communicate beyond what the section's own state note already says.
  - [ ] Empty entries list (`entries.length === 0`): a plain paragraph — `Ще немає заявлених команд.` (this **is** the `NO_TEAMS` constant's real, intended context — `src/lib/empty-states.ts`'s own comment already earmarks it for "an empty schedule tab"-style contexts; use `<EmptyState {...NO_TEAMS} />` here, the one place in the codebase so far where reusing it is actually correct).
