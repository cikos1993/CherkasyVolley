...
- [ ] **Task 9 — Commit** — `feat(ui): reusable ConfirmDialog / Skeleton / EmptyState / toast helper (Story 2.2)`. Commit to `main`; push deploys to Vercel (view-only change, no migration). Include the `admin-role-controls.tsx` migration and the 3 `EmptyState` call-site refactors in the same commit (they are part of "formalise the reusable version").
